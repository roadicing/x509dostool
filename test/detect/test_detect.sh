...
if [ ! -d "$cert_path" ]; then
    ...
    test_commands=(
        "x509dostool generate -out certs/01-a.pem test1 -m 0x7FFFFF --balanced --compressed"
        "x509dostool generate -out certs/01-b.pem test1 -m 0x5FFFFFFF"
        "x509dostool generate -out certs/02.pem test2 -m 74 -t 233 --compressed"
        "x509dostool generate -out certs/03-a.pem test3 --balanced"
        "x509dostool generate -out certs/03-b.pem test3 -p '(2**127-1)**2' --balanced"
        "x509dostool generate -out certs/04-a.pem test4 -p '2**86243-1' -algo rsa"
        "x509dostool edit -in certs/04-a.pem -out certs/04-a.pem --pubout"
        "x509dostool generate -out certs/04-b.pem test4 -p '2**86243-1'"
        "x509dostool generate -out certs/05.pem test5 -sans 60000"
        "x509dostool generate -out certs/06.pem test6"
        "x509dostool generate -out certs/07.pem test7"
        "x509dostool generate -out certs/08.pem test8 -num 4"
        "x509dostool generate -out certs/09.pem test9 -num 32 --mapping"
        "x509dostool generate -out certs/10.pem test10 "
    )
