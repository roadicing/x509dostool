...
test_edit_commands=(
    "x509dostool generate -out test.crt test0"
    "x509dostool edit -in test.crt -out edited_test.crt -outform der"
    "x509dostool edit -in test.crt -out edited_test.crt tbs ver -ver 1023"
    "x509dostool edit -in test.crt -out edited_test.crt tbs sn -sn 1023"
    "x509dostool edit -in test.crt -out edited_test.crt tbs sig -algo 1.2.1023"
...
for cmd in "${test_edit_commands[@]}"; do
    ...
    if [[ "$cmd" == "x509dostool edit"* ]] && [[ ! -f "edited_test.crt" ]]; then
        failure_count=$((failure_count + 1))  # increment failure count
        echo "error: certificate not edited by command: $cmd"
        continue
    fi
